...
def run_coll_post(current_user):
    ...
    for report_path in selected_paths:
        ...
        outcome = post_confirmed_report(report_path, report_data)
        if not outcome.ok:
    ...
    while True:
        ...
        while True:
            ...
            if confirm == "b":
                break
            if confirm == "r":
                apply_start_approval(report_path, report_data, "return")
                print("Collection list returned — it must be regenerated by the salesman.")
                prompt_continue()
                return
            if confirm == "c":
                apply_start_approval(report_path, report_data, "cancel")
                print("Collection list cancelled.")
                prompt_continue()
                return
            if confirm in ("y", "yes"):
